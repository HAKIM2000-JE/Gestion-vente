import React, { useState, useEffect } from 'react';
import './payement.css';
import { useStateValue } from "./StateProvider";
import CheckoutProduct from "./CheckoutProduct";
import { Link, useHistory } from "react-router-dom";
import { CardElement, useStripe, useElements } from "@stripe/react-stripe-js";
import CurrencyFormat from "react-currency-format";
import { totalPanier } from "./reducer";
import axios from './axios';
import { db } from "./firebase";

function Payment() {
    const [{ panier, user }, dispatch] = useStateValue();
    const history = useHistory();

    const stripe = useStripe();
    const elements = useElements();

    const [succeded, setSucceeded] = useState(false);
    const [processing, setProcessing] = useState("");
    const [error, setError] = useState(null);
    const [disabled, setDisabled] = useState(true);
    const [clientSecret, setClientSecret] = useState(true);

    useEffect(() => {
        // generate the special stripe secret which allows us to charge a customer
        const getClientSecret = async () => {
            const response = await axios({
                method: 'post',
                // Stripe expects the total in a currencies subunits
                url: `/payments/create?total=${totalPanier(panier) * 100}`
            });
            setClientSecret(response.data.clientSecret)
        }

        getClientSecret();
    }, [panier])

    console.log('THE SECRET IS >>>', clientSecret)
    console.log( user)

    const handleSubmit = async (event) => {
        // do all the fancy stripe stuff...
        event.preventDefault();
        setProcessing(true);

        const payload = await stripe.confirmCardPayment(clientSecret, {
            payment_method: {
                card: elements.getElement(CardElement)
            }
        }).then(({ paymentIntent }) => {
            // paymentIntent = payment confirmation

            db
                .collection('users')
                .doc(user?.uid)
                .collection('orders')
                .doc(paymentIntent.id)
                .set({
                    panie: panier,
                    amount: paymentIntent.amount,
                    created: paymentIntent.created
                })

            setSucceeded(true);
            setError(null)
            setProcessing(false)

            dispatch({
                type: 'VIDER_PANIER'
            })

            history.replace('/orders')
        })

    }

    const handleChange = event => {
        // Listen for changes in the CardElement
        // and display any errors as the customer types their card details
        setDisabled(event.empty);
        setError(event.error ? event.error.message : "");
    }

    return (
        <div className="payement">
            <div className="payement_container">
                <h1>
                    Payement( <Link to="/checkout" >
                        {panier.length} Articles
                </Link>)
             </h1>

                {/* payement section - delevery adresse*/}
                <div className="payement_section">
                    <div className="payement_title">
                        <h3> Adresse D'achat</h3>
                    </div>
                    <div className="payement_adresse">
                        <p>{user?.email}</p>
                        <br />
                        <p>Quartier Essalam Meknes</p>
                        <p>Maroc</p>
                    </div>
                </div>

                {/* payement section - Review items */}
                <div className="payement_section">
                    <div className="payement_title">
                        <h3>Articles Achetés</h3>
                    </div>

                    {/* Here we gonna display all our bought product */}

                    <div className="payement_itmes">
                        {panier.map(item => (
                            <CheckoutProduct
                                id={item.id}
                                title={item.title}
                                image={item.image}
                                price={item.price}
                                rating={item.rating}
                            />
                        ))}
                    </div>

                </div>

                {/* payement section - payement-method*/}
                <div className="payement_section">
                    <div className="payement_title">
                        <h3>Information de la carte</h3>
                    </div>

                    <div className="payement_details">
                        {/*here we gonna have stripe stuff */}
                        <form onSubmit={handleSubmit} >
                            <CardElement onChange={handleChange} />
                            <div className="payement_price_container">
                                <CurrencyFormat
                                    renderText={(value) => (
                                        <>
                                            <h3> Total à payer : {value}</h3>
                                            
                                        </>
                                    )}
                                    decimalScale={2}
                                    value={totalPanier(panier)}
                                    displayType={"text"}
                                    thousandSeparator={true}
                                    prefix={"$"}



                                />
                                <button className="achat_btn" disabled={processing || disabled || succeded}>
                                    <span>{processing ? <p>Processing</p> : "Acheter"}</span>
                                </button>
                            </div>

                            {/* Error */}
                            {
                                error && <div>{error}</div>
                            }
                        </form>

                    </div>

                </div>

            </div>

        </div>
    )
}

export default Payment